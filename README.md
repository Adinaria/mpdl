...
## 📝 Логика проекта
1) Роли administrator и human является ролями по умолчанию. Их нельзя удалить и изменить. Так как каждый пользователь который создается, получает роль human.
   А создавать роли и юзеров может только админ
2) Роль нельзя удалить если она привязана к пользователю
3) По умолчанию сидом создается пользователь admin с ролью administrator. Его креды: email - admin@gmail.com, password - admin@gmail.com
...
Кешированием можно управлять из конфига cache_entity, можно его включить для определенных сущностей и выключать.
